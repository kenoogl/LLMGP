#!/bin/bash
# Target app template: capture one unrecorded previous Codex rollout on SessionStart.
...
SESSION_ID=""
CWD=""
LOG_PATH="${RC_SESSION_CAPTURE_PREVIOUS_HOOK_LOG:-$REVIEWCOMPASS_DIR/.reviewcompass/runtime/session-record-capture-previous-codex.jsonl}"
CODEX_ROOT="${CODEX_SESSIONS_ROOT:-$HOME/.codex/sessions}"
REVIEWCOMPASS_JQ="${REVIEWCOMPASS_JQ:-jq}"
CAPTURE_TOOL="${REVIEWCOMPASS_CAPTURE_PREVIOUS_CODEX_TOOL:-$REVIEWCOMPASS_DIR/tools/session-record-capture-previous-codex.py}"
...
[ ! -x "$REVIEWCOMPASS_PYTHON" ] && { log_event "no_python"; exit 0; }
[ ! -f "$CAPTURE_TOOL" ] && { log_event "no_capture_tool"; exit 0; }

cd "$REVIEWCOMPASS_DIR" || exit 0

...
2. 最初の命令文は、変数またはケースパラメータを1つ指定する`load`のみとする（`load`の引数は、参照する変数名であり、他の命令文への参照ではない）
3. 2番目以降の各命令文は、演算子をランダムに選び、その演算子が必要とする引数の数（0個・1個・2個）に応じて、引数それぞれについて「自分より前に生成された命令文の出力」または「自由係数のプレースホルダ」のいずれかをランダムに選ぶ（＝自分より後に生成される命令文は参照できない）

**受け入れ基準**：
- 個体数を指定でき、その数だけ項が生成される
- 各項の最初の命令文は必ず`load`であり、変数またはケースパラメータを指定する引数を持つ
- 各命令文の引数のうち、他の命令文を参照するものは、必ず自分より前に生成された命令文を指す
- 各命令文は、自由係数のプレースホルダを引数に持つことができる（term-representation FR3準拠）

### FR2: 交叉（crossover）
2つの親の項（命令文数がそれぞれa, b）から、先頭の命令文を入れ替える1点交叉により、2つの子の項を生成する。

1. 入れ替える命令文数kを、1以上min(a, b)以下の範囲でランダムに選ぶ
2. 子1＝親2の先頭k個の命令文＋親1の先頭k個を除いた残りの命令文
3. 子2＝親1の先頭k個の命令文＋親2の先頭k個を除いた残りの命令文

**受け入れ基準**：
- 2つの親項を入力すると、2つの子項が出力される
- 子1の先頭k個の命令文は、親2の先頭k個の命令文と一致する。子2の先頭k個の命令文は、親1の先頭k個の命令文と一致する
- 出力される子項は、命令文数がもとの親と一致し、保持番号・引数の整合（FR1の受け入れ基準）が保たれている

### FR3: 変異（mutation）
親の項から、命令文を1つランダムに選び、その命令文をFR1と同じ生成方法（ただし引数は、選んだ命令文より前に位置する命令文の中から選ぶ）で作った新しい命令文に差し替える。

**受け入れ基準**：
- 1つの親項を入力すると、1つの子項が出力される
- 出力される子項は、選んだ命令文が、FR1と同じ生成方法で作られた新しい命令文に置き換えられている

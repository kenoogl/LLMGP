...
The project-level check in (a) is operationalized concretely by §7 check 1 (purpose) and check 4 (the `d_dt`/steady-state constraint from intent, the only project-level `NON_GOALS.md` item directly applicable to this feature). Other `NON_GOALS.md` items (no CFD simulation, no LLM training) are not directly applicable to term-variation-operators; check only that the target does not contradict them, and do not require the target to restate them.

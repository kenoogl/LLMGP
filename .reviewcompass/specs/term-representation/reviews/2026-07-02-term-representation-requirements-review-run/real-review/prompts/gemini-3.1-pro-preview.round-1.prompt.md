...
- purpose: `term-representation` defines the GP gene/term data structure as the foundation feature of LLMGP's LLM-less GP core, per §1-2.
- responsibility_boundaries: see §7 check 2 (`boundary_transfer`) and §8 (Out of Scope).
- acceptance_criteria: see §7 checks 1, 3, 5.
- forbidden_actions: see §7 checks 4, 5 (no `d_dt`, no hard-coded variable list).
- unresolved_or_design_deferred_items: the concrete crossover/mutation algorithm, the DE fitness function's L1-penalty weighting, and the importance-normalization formula are deferred to `term-variation-operators`, `equation-fitting`, and `term-importance` respectively, per the approved feature-partitioning (§6.4).
- intended_target_phase_transfer: the design stage for `term-representation` is expected to choose the concrete Julia data structure and API while preserving these acceptance criteria and boundaries unchanged, per `main-preanalysis.md`'s `intended_target_phase_transfer` note.

## 4. Required Disciplines

The following is background policy text explaining what a "vertical intent transfer" check means for a requirements-phase review. It is provided so you can judge the target document `requirements.md` directly against this policy (in addition to the required checks in §7), not so you evaluate this criteria file itself. Do not produce findings about this criteria file or about §7; all findings must be about the target document named in §5.
...
1. **purpose_transfer**: Does the target's overview correctly state that `term-representation` defines the GP gene/term data structure as the foundation for the other core features, consistent with intent's stated purpose? Failure condition: the overview omits or misstates this foundational role.
2. **boundary_transfer**: Does the target's scope-out list correctly exclude crossover/mutation/seeding, DE coefficient optimization with L1 penalty, importance scoring, and CSV/case-parameter data preprocessing? The crossover/mutation/seeding, coefficient-optimization, and importance-scoring boundaries are grounded in the feature-partitioning excerpt in §6.4; the CSV/case-parameter preprocessing boundary is grounded in the user-directed decisions in §3. Failure condition: any of these responsibilities is implicitly claimed by the target's functional requirements, or a boundary is missing from the scope-out list.
3. **acceptance_criteria_transfer**: Do the target's functional requirements operationalize intent's "GPプロセス自体の精査点" statement (expressive terms with free internal coefficients, no `d_dt`) into concrete, checkable acceptance criteria? Failure condition: the requirement describing allowed operators omits the `d_dt` exclusion, or the requirement describing free coefficients is missing or contradicts intent.
